...
2. Register the client's intake: business type, target public function,
   prior filing history in the United Arab Emirates if any.
3. Run the advisor in read-only mode against individual federal and emirate-level e-procurement channels (e.g. federal entities and Abu Dhabi/Dubai government procurement portals), plus In-Country Value (ICV) certification for tenders where ICV scoring applies.
...
- named referral relationship with UAE-licensed counsel or a
...
a working referral relationship with UAE-licensed counsel or a
registered agent for whatever licensed representation the law of
the United Arab Emirates requires for actual public-procurement filings.
