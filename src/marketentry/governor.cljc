(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a United Arab Emirates-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. United Arab Emirates-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ae-entity?
                                       true` (near-universal for United Arab Emirates
                                       ...
                                       Grounded in UAE trade license typically
                                       required for federal awards (contracting
                                       authorities / MoF).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(TRADE-LICENSE/統一資格/Federal e-procurement登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ae-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ae-entity? true`, INDEPENDENTLY verify
  `:has-ae-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ae-entity?` ground truth (most United Arab Emirates public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ae-entity? e))
                 (not (true? (:has-ae-entity? e))))
        [{:rule :ae-entity-missing
          :detail (str subject " はアラブ首長国連邦居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- trade-license-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-trade-license? true`, INDEPENDENTLY check
  `:trade-license-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-trade-license? e))
                 (not (true? (:trade-license-verified? e))))
        [{:rule :trade-license-unverified
          :detail (str subject " はTRADE-LICENSE確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ae-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (trade-license-unverified-violations request st)
