...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (ARE, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ae-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (ae-entity-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up trade-license-unverified) ==")
    ...
    (println "== filing/submit eng-5 (trade-license-unverified -> HARD hold) ==")
